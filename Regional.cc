...
struct Parse_cubeface00_step : ConfigParser {  // This is the first face of a cubed sphere.
   Parse_cubeface00_step( const Projection& p, const Grid::Config& config ) {
        std::vector<double> nw;
        int face_no;
        valid = config.get( "nx", x.N ) && config.get( "cubed_sphere_face", face_no ) &&
                config.get( "lonlat(xmin,ymax)", nw);
...
// y is eta ;
        ...
        x.min = - M_PI / 4.0;
        y.max = M_PI / 4.0;
        x.max = M_PI / 4.0 - x.step;
        y.min = - M_PI / 4.0 + y.step;
        x.endpoint = false;
        y.endpoint = false;
    }
};
...
bool ConfigParser::parse( const Projection& projection, const Grid::Config& config, Parsed& x, Parsed& y ) {
    ...
    if ( ConfigParser::parse<Parse_cubeface00_step>( projection, config, x, y ) ) {
        return true;
    }
