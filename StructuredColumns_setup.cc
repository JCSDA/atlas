...
void StructuredColumns::setup( const grid::Distribution& distribution, const eckit::Configuration& config ) {
    ...
    atlas::Log::info() << "StructuredColumns bounds without halo "  << mpi_rank << " " << j_begin_ << " " << j_end_ << " " << i_begin_ << " " << i_end_ << std::endl;
    atlas::Log::info() << "StructuredColumns bounds with halo "  << mpi_rank  << " " << j_begin_halo_ << " " << j_end_halo_ << " " << i_begin_halo_ << " " << i_end_halo_ << std::endl;
 ...
 //   atlas::Log::info() << "StructuredColumns bounds compute i j x y "  << mpi_rank << " " << compute_i << " " << compute_j << " " << compute_x << " " << compute_y << std::endl;
    atlas::Log::info() << "StructuredColumns bounds grid_-> ny() "  << mpi_rank << " " << grid_->ny() << std::endl;
    for ( idx_t j = 0; j < grid_->ny(); ++j ) {
      atlas::Log::info() << "StructuredColumns bounds j grid_-> nx() "  << mpi_rank << " " << j << " " << grid_->nx(j) << std::endl;
    }
    ...
    auto compute_g = [this, &global_offsets, &compute_i, &compute_j]( idx_t i, idx_t j ) -> gidx_t {
        ...
        ii = compute_i( i, jj );
        if ( jj != j ) {
            atlas::Log::info() << "StructuredColumns setup jj not j)"  <<  atlas::mpi::comm().rank()  << jj << j << std::endl;
