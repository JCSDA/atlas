...
StructuredMeshGenerator::StructuredMeshGenerator( const eckit::Parametrisation& p ) {
    ...
    Log::info() << "Atlas entered StructuredMeshGenerator( const eckit::Parametrisation& p ) " <<  atlas::mpi::comm().rank() << std::endl;
    ...
    if ( p.get( "partitioner", partitioner ) ) {
        Log::info() << "Atlas exited StructuredMeshGenerator( const eckit::Parametrisation& p ) partitioner from p " <<   partitioner << std::endl;
    ...
    Log::info() << "Atlas exited StructuredMeshGenerator( const eckit::Parametrisation& p ) " <<  atlas::mpi::comm().rank() << " " << options  << std::endl;
